...
def load_all_models():
    ...
    try:
        # Load crop model with error handling for version compatibility
        ...
        if os.path.exists(crop_model_file):
            try:
                ...
                if 'numpy._core' in str(e):
                    print("⚠️ Skipping crop_model_v1.pkl due to numpy._core compatibility issue")
                else:
                    print(f"⚠️ Error loading crop_model_v1.pkl: {e}")
        
        # Load crop label encoder (use latest version) with error handling
        ...
        if os.path.exists(crop_encoder_file):
            try:
                ...
                if 'numpy._core' in str(e):
                    print("⚠️ Skipping crop_encoder.pkl due to numpy._core compatibility issue")
                else:
                    print(f"⚠️ Error loading crop_encoder.pkl: {e}")
        else:
            ...
            if os.path.exists(crop_encoder_file_old):
                try:
                    ...
                    print("✅ Loaded crop_label_encoder_v1.pkl (fallback)")
                except (ModuleNotFoundError, ImportError) as e:
                    if 'numpy._core' in str(e):
                        print("⚠️ Skipping crop_label_encoder_v1.pkl due to numpy._core compatibility issue")
                    else:
                        print(f"⚠️ Error loading crop_label_encoder_v1.pkl: {e}")
        ...
        if os.path.exists(scaler_file):
            try:
                ...
                if 'numpy._core' in str(e):
                    print("⚠️ Skipping scaler_v1.pkl due to numpy._core compatibility issue")
                else:
                    print(f"⚠️ Error loading scaler_v1.pkl: {e}")
        
        # Load fertilizer model with error handling
        ...
        if os.path.exists(fertilizer_model_file):
            try:
                ...
                print("✅ Loaded fertilizer_model_v1.pkl")
            except (ModuleNotFoundError, ImportError) as e:
                if 'numpy._core' in str(e):
                    print("⚠️ Skipping fertilizer_model_v1.pkl due to numpy._core compatibility issue")
                else:
                    print(f"⚠️ Error loading fertilizer_model_v1.pkl: {e}")
        
        # Load profit/yield model with error handling
        ...
        if os.path.exists(yield_model_file):
            try:
                ...
                print("✅ Loaded yield_model_v1.pkl")
            except (ModuleNotFoundError, ImportError) as e:
                if 'numpy._core' in str(e):
                    print("⚠️ Skipping yield_model_v1.pkl due to numpy._core compatibility issue")
                else:
                    print(f"⚠️ Error loading yield_model_v1.pkl: {e}")
            ...
            for file in os.listdir(encoders_dir):
                if file.endswith('.pkl'):
                    ...
                    try:
                        ...
                        if 'numpy._core' in str(e):
                            print(f"⚠️ Skipping encoder {file} due to numpy._core compatibility issue")
                        else:
                            print(f"⚠️ Failed to load encoder {file}: {e}")
                    except Exception as e:
                        print(f"⚠️ Failed to load encoder {file}: {e}")
        
        # Load fertilizer encoders with error handling
        for encoder_type in ['crop', 'fertilizer']:
            ...
            if os.path.exists(encoder_file):
                ...
                try:
                    ...
                    print(f"✅ Loaded fertilizer_{encoder_type}_encoder_v1.pkl")
                except (ModuleNotFoundError, ImportError) as e:
                    if 'numpy._core' in str(e):
                        print(f"⚠️ Skipping fertilizer_{encoder_type}_encoder_v1.pkl due to numpy._core compatibility issue")
                    else:
                        print(f"⚠️ Error loading fertilizer_{encoder_type}_encoder_v1.pkl: {e}")
                except Exception as e:
                    print(f"⚠️ Error loading fertilizer_{encoder_type}_encoder_v1.pkl: {e}")
